...
# 检查安装包
if [ ! -f "/tmp/Percona-Server-5.7.35-38-Linux.x86_64.glibc2.12.tar.gz" ]; then
    print_error "安装包不存在：/tmp/Percona-Server-5.7.35-38-Linux.x86_64.glibc2.12.tar.gz"
fi
...
# 解压文件
tar zxf Percona-Server-5.7.35-38-Linux.x86_64.glibc2.12.tar.gz -C "${TEMP_DIR}" || print_error "解压安装包失败"
