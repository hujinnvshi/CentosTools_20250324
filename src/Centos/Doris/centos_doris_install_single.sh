...
INSTALL_PACKAGE="/tmp/apache-doris-2.1.6-bin-x64.tar.gz"
INSTALL_DIR="/tmp/apache-doris-2.1.6-bin-x64"
...
if [ ! -d "/tmp/apache-doris-2.1.6-bin-x64/fe" ] || [ ! -d "/tmp/apache-doris-2.1.6-bin-x64/be" ]; then
    print_error "Doris 安装包未找到，请确保已解压到 /tmp/apache-doris-2.1.6-bin-x64"
fi
cp -r /tmp/apache-doris-2.1.6-bin-x64/fe/* ${FE_HOME} || print_error "移动 FE 文件失败"
cp -r /tmp/apache-doris-2.1.6-bin-x64/be/* ${BE_HOME} || print_error "移动 BE 文件失败"
