...
check_arm_architecture() {
    print_message "检查系统架构..."
    if [ "$(uname -m)" != "aarch64" ]; then
        print_error "此脚本仅支持 ARM64 架构"
        exit 1
    fi
    print_message "确认为 ARM64 架构"
}

# 安装 ARM 特定依赖
install_arm_dependencies() {
    print_message "安装 ARM 架构依赖..."
    
    # 安装基础依赖
    yum install -y gcc gcc-c++ make cmake zlib-devel openssl-devel snappy-devel \
        bzip2-devel protobuf-devel maven autoconf automake libtool curl

    # ARM 特定依赖
    yum install -y aarch64-linux-gnu-gcc
}

# 配置 Native 库
configure_native_libs() {
    print_message "配置 Hadoop Native 库..."
...
EOF
    ...
    # 验证 Native 库
    ${HADOOP_HOME}/bin/hadoop checknative -a || print_warning "Native 库可能不完整"
}
